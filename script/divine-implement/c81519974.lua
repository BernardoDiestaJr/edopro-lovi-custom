--Divine Implement - Sealing Amulets
local s,id=GetID()
...
s.listed_series={0x1f9,0x1f7}
s.listed_names={0x1f9,id}

function s.effcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	if Duel.CheckReleaseGroupCost(tp,Card.IsRace,1,true,nil,nil,RACE_SPELLCASTER) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		local g=Duel.SelectReleaseGroupCost(tp,Card.IsRace,1,1,true,nil,nil,RACE_SPELLCASTER)
...
function s.thfilter(c)
	return c:IsSetCard(0x1f7) and c:IsSpellTrap() and c:IsAbleToHand() and not c:IsCode(id)
end
function s.spfilter(c,e,tp)
	return (c:IsCode(0x1f9) or c:ListsCode(0x1f9)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 or op==3 then
		--Add 1 "Divine Implement" Spell/Trap from your Deck or GY to your hand.
