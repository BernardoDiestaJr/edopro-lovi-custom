...
function s.spfilter(c,e,tp,mc,pg)
	...
	return ((c:IsSetCard(0x1fd) and c:IsMonster() and c:IsType(TYPE_XYZ)) or c:IsSetCard(0x1fc) and c:IsMonster() and c:IsType(TYPE_XYZ)) and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0
...
function s.xyzfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsRace(RACE_BEAST|RACE_FAIRY|RACE_SEASERPENT)
end
