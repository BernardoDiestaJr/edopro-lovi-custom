--Mystikrios Rising Dissension
local s,id=GetID()
function s.initial_effect(c)
	--Destroy 1 monster and search 1 "Mystikrios" Spell/Trap or 1 "Faerie Avalontino"
...
s.listed_names={id, 815199721, 815199729}

function s.thfilter(c,alt)
	return c:IsAbleToHand() and (c:IsSetCard(0x1fc) and c:IsSpellTrap() and not c:IsCode(id) or (alt and c:IsCode(815199729)))
end

function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) end
	local alt=Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,815199721),tp,LOCATION_ONFIELD,0,1,nil)
...
function s.tdfilter(c)
	return c:IsType(TYPE_FUSION|TYPE_XYZ) and c:IsAbleToExtra()
end
