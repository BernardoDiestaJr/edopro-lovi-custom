...
function s.exspfilter(c,e,tp,mc)
	return c:IsSetCard(0x1fc) and c:IsType(TYPE_XYZ) and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0 and mc:IsCanBeXyzMaterial(c,tp)
		...
		if (#pg<=0 or (#pg==1 and pg:IsContains(sc))) and Duel.IsExistingMatchingCard(s.exspfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,sc)
			...
			if xyzc then
				...
				aux.DelayedOperation(xyzc,PHASE_END,id,e,tp,
					function(ag) Duel.Remove(ag,POS_FACEUP,REASON_EFFECT) end,
...
function s.setfilter(c)
	return c:IsSetCard(0x1fc) and c:IsQuickPlaySpell() and c:IsSSetable()
end
