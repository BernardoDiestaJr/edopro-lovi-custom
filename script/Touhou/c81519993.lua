...
function s.thfilter(c)
	return (c:IsSetCard(0x1fa) and c:IsSpell() and not c:IsMonster() and not c:IsCode(id) or c:IsLevelBelow(6) and c:IsType(TYPE_SPIRIT)) and c:IsAbleToHand()
end
...
function s.rescon(sg,e,tp,mg)
	return sg:FilterCount(Card.IsSpell,nil,0x1fa)<=1 and sg:FilterCount(Card.IsType,nil,TYPE_SPIRIT) and sg:FilterCount(Card.IsLevelBelow,nil,6)<=1
end
