...
function s.initial_effect(c)
	...
	--If this card is Link Summoned: You can place 1 "Shrine of Paradise" from your hand, Deck or GY face-up in your Field Zone.
	...
	e1:SetTarget(s.plthtg)
	e1:SetOperation(s.plthop)
	c:RegisterEffect(e1)
...
function s.plthfilter(c,tohand_chk)
	return (c:IsCode(81519972) and not c:IsForbidden()) or (tohand_chk and c:IsSetCard(0x1f8) and c:IsAbleToHand() and c:IsLocation(LOCATION_DECK))
end

function s.plthtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local tohand_chk=Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,81519972),tp,LOCATION_ONFIELD,0,1,nil)
...
function s.tgfilter(c)
	return c:IsRace(RACE_ILLUSION) and c:IsLevelBelow(10) and c:IsAbleToGrave()
end
