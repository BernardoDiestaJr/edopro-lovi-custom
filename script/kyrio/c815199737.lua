...
function s.deckmatfilter(c)
	return c:IsRace(RACE_SEASERPENT) and c:IsReleasableByEffect()
end
...
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	local params1={lvtype=RITPROC_EQUAL,filter=aux.FilterBoolFunction(Card.IsRace,RACE_SEASERPENT),location=LOCATION_GRAVE,matfilter=aux.FilterBoolFunction(Card.IsRace,RACE_SEASERPENT)}
	local params2={lvtype=RITPROC_EQUAL,filter=aux.FilterBoolFunction(Card.IsRace,RACE_SEASERPENT),location=LOCATION_DECK,matfilter=aux.FilterBoolFunction(Card.IsRace,RACE_SEASERPENT),extrafil=s.extragroup,extraop=s.extraop}
